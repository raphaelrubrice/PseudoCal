...
class TempScaling_LS(nn.Module):
    def __init__(self, class_num):
        super(TempScaling_LS, self).__init__()
        self.class_num = class_num

    def find_best_T(self, logits, labels):
        #nll_criterion = nn.CrossEntropyLoss(reduction='none')
        nll_criterion = CrossEntropyLabelSmooth(num_classes=self.class_num, reduction='none')
        def eval(x):
            "x ==> temperature T"
...
def VectorOrMatrixScaling(logits, labels, outputs_target, labels_target, cal_method=None):
    ...
    after_calibration_bs = calculate_brier_score(final_output, labels_target).item()
    # print('Optimal temperature: %.3f' % self.temperature.item())
    # print('After temperature - NLL: %.3f, ECE: %.3f' % (after_calibration_nll, after_calibration_ece))
    return after_calibration_ece, after_calibration_nll, after_calibration_bs
...
def calculate_brier_score(softmax_output, ground_truth, LOGIT=True):
    """
    Calculates the mean Brier score given the softmax output and the ground truth labels.

    :param softmax_output: A PyTorch tensor of shape (n_samples, n_classes) containing the softmax output
    :param ground_truth: A PyTorch tensor of shape (n_samples,) containing the ground truth labels
    :return: A scalar value representing the mean Brier score over all classes
    """
    if LOGIT:
        softmax_output = nn.Softmax(dim=-1)(softmax_output).detach().cpu().numpy()
    else:
    ...
    for i in range(n_classes):
        brier_scores[i] = brier_score_loss((ground_truth == i).astype(int), softmax_output[:, i])
